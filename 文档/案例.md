...
注意看清楚要修改的是`headers`还是`header`中的值

修改请求头`'Content-Type': 'multipart/form-data'`，并且数据要放在序列化的表单FormData中
...
uploadFile(option){
   ...
   console.log('formData', formData)
    axios({
      method: 'post',
      url: 'http://10.18.100.110:8080/factory/manage/add',
      data: formData,// 文件流格式的数据
      headers: {
...
修改请求头`'Content-Type': 'application/x-www-form-urlencoded'`
...
<div class="table-view">
  <div ref="action-bar" class="action-bar">
    <slot name="action-bar" :selectedRowKeys="selectedRowKeys" :form="form" />
  </div>
  <el-table
    ref="table"
    v-loading="loading"
